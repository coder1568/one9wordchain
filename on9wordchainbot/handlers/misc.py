...
from .. import GlobalState, bot, dp, db
...
async def cmd_start(message: types.Message) -> None:
    await message.reply(
        (
            "Welcome! I'm a word chain game bot for Telegram groups.\n"
            "Add me to your group and start playing awesome word games now!"
        ),
...
async def new_member(message: types.Message) -> None:
    if any(user.id == bot.id for user in message.new_chat_members):  # self added to group
        await message.reply(
            "Thanks for inviting me! Let's play word chain games. Use /startclassic to begin!",
            reply=False
        )
    elif message.chat.id == OFFICIAL_GROUP_ID:
        await message.reply(
            (
                "Welcome everyone! Ready to play word chain?\n"
                "Type /startclassic to start your first game!"
            ),
            allow_sending_without_reply=True
        )
...
async def inline_handler(inline_query: types.InlineQuery):
    ...
    res = []
    # Get words that start with the query text
    matching_words = Words.starts_with(text.lower())
